...
#define MIN_CHUNKS_SIZE(data_size, chunk_size)  (MIN_NUM_CHUNKS(data_size, chunk_size) * (chunk_size))
        ...
        void HandTracker::setupStream(sensekit_streamset_t setHandle, sensekit_stream_desc_t depthStreamDesc)
        {
            ...
            m_pluginService.create_stream(m_streamSet, desc, pluginCallbacks, &m_handStream);
            ...
            if (rc != SENSEKIT_STATUS_SUCCESS)
            {
                ...
                throw logic_error("Could not add a stream with desired description");
            }
        ...
        void HandTracker::updateTracking(sensekit_depthframe_t depthFrame)
        {
            ...
            m_pluginService.cycle_bin_buffers(m_handBinHandle, &nextBuffer);
        ...
        void HandTracker::connection_added(sensekit_streamconnection_t connection)
        {
            if (m_handBinHandle == nullptr)
            {
                ...
                sensekit_frame_t* nextBuffer = nullptr;
                m_pluginService.create_stream_bin(m_handStream, binLength, &m_handBinHandle, &nextBuffer);
                setNextBuffer(nextBuffer);
                m_pluginService.link_connection_to_bin(connection, m_handBinHandle);
            }
        }
        ...
        void HandTracker::connection_removed(sensekit_streamconnection_t connection)
        {
            ...
            bool hasConnections;
            m_pluginService.bin_has_connections(m_handBinHandle, &hasConnections);
            if (!hasConnections)
            {
                m_pluginService.destroy_stream_bin(m_handStream, &m_handBinHandle, &m_currentHandBuffer);
